...
LastEditTime: 2021-11-06 11:04:48
'''
...
class Data:
    def __init__(self, dataset=None, add_reverse_relation=False):
        ...
        self.timestamps = self._get_timestamps(self.data)# 获取所有的时间戳
        

        ent_time_set = []
        for d in self.data:
            ent_time_set += [(d[0],d[3]), (d[2],d[3])]
        
        self.ent_time_set = sorted(set(ent_time_set), key=lambda x: (int(x[0]), int(x[1])))  #排序:时间，尾市体
    ...
    def get_rel_dict(self):
        ...
        for event in self.train_data:
            rel_dict[int(event[1])].append((int(event[2]), int(event[3]))) #
        for ss in rel_dict:
            rel_dict[ss] = list(set(rel_dict[ss]))
    ...
    def get_spt2o(self, dataset: str):
        ...
        if dataset == 'train':
            ...
            events = np.concatenate([self.train_data, self.valid_data], axis=0)
        elif dataset == 'test':
            # events = self.test_data
            events = np.concatenate([self.train_data, self.valid_data, self.test_data], axis=0)
        else:
...
class NeighborFinder:
    def __init__(self, adj, rel_adj=None, sampling=1, max_time=366 * 24, num_entities=None, num_relations=None, weight_factor=1, time_granularity=24,name='YAGO1830'):
        """
        Params
        ------
        ...
        if rel_adj!=None: 
            rel_off_set_l, rel_off_set_t_l, rel_node_idx_l = self.rel_init_off_set(rel_adj, max_time, num_relations)
            self.rel_off_set_l = rel_off_set_l
            self.rel_off_set_t_l = rel_off_set_t_l
            self.rel_node_idx_l = rel_node_idx_l
    ...
    def rel_init_off_set(self, adj, max_time, num_relations):
        """
        ...
        """
        try:
            off_set_t_l = np.load('./process_data/'+self.name+'_rel_off_set_t_l.npy')
            rel_node_idx_l = np.load('./process_data/'+self.name+'_rel_node_idx_l.npy')
            off_set_l = np.load('./process_data/'+self.name+'_rel_off_set_l.npy')
        except:
            off_set_l = [0]
            off_set_t_l = []
            rel_node_idx_l = []
            if isinstance(adj, dict):
                for i in range(num_relations):
                    curr = adj[i]
                    curr = sorted(curr, key=lambda x: (int(x[1]), int(x[0])))  #排序:时间，尾市体
                    curr_ts = [x[1] for x in curr]  #时间戳集合
                    rel_node_idx_l.extend([x[0] for x in curr]) #尾市体集合
                    off_set_l.append(len(rel_node_idx_l))
                    ...
                    # 找出cut_time放在curr_ts的第几个位置上，才能保证顺序不会被打乱，就是相当于找出cut_time应该在第几个位置，等价于找出前面有多少个数字呗

            off_set_t_l = np.array(off_set_t_l) #记录第多少个对应第i个实体
            rel_node_idx_l = np.array(rel_node_idx_l)
            off_set_l = np.array(off_set_l) #记录每个batch对应的尾市体的数目
            

            np.save('./process_data/'+self.name+'_rel_node_idx_l.npy',rel_node_idx_l)
            np.save('./process_data/'+self.name+'_rel_off_set_t_l.npy',off_set_t_l)
            np.save('./process_data/'+self.name+'_rel_off_set_l.npy',off_set_l)


        
        return off_set_l,off_set_t_l,rel_node_idx_l
    ...
    def init_off_set(self, adj, max_time, num_entities):
        ...
        """
        try:
            n_idx_l = np.load('./process_data/'+self.name+'_n_idx_l.npy')
            n_ts_l = np.load('./process_data/'+self.name+'_n_ts_l.npy')
            e_idx_l = np.load('./process_data/'+self.name+'_e_idx_l.npy')
            off_set_l = np.load('./process_data/'+self.name+'_off_set_l.npy')
            off_set_t_l = np.load('./process_data/'+self.name+'_off_set_t_l.npy')
        except:
            ...
            np.save('./process_data/'+self.name+'_n_idx_l.npy',n_idx_l)
            np.save('./process_data/'+self.name+'_n_ts_l.npy',n_ts_l)
            np.save('./process_data/'+self.name+'_e_idx_l.npy',e_idx_l)
            np.save('./process_data/'+self.name+'_off_set_l.npy',off_set_l)
            np.save('./process_data/'+self.name+'_off_set_t_l.npy',off_set_t_l)
    ...
    def get_temporal_neighbor(self, src_idx_l, cut_time_l, base_time_l, num_neighbors=20,p2o=None,query_idx_l=None,batch_idx_l=None,flag=None):
        """
        ...
        if self.sampling == -1:
            ...
            full_ngh_t = []
            full_ngh_edge = []
        for i, (src_idx, cut_time, base_time, batch_idx) in enumerate(zip(src_idx_l, cut_time_l, base_time_l, batch_idx_l)):
            query_idx = query_idx_l[batch_idx] #
            neighbors_idx = self.node_idx_l[self.off_set_l[src_idx]:self.off_set_l[src_idx + 1]]  #
            neighbors_ts = self.node_ts_l[self.off_set_l[src_idx]:self.off_set_l[src_idx + 1]]
            neighbors_e_idx = self.edge_idx_l[self.off_set_l[src_idx]:self.off_set_l[src_idx + 1]]
            ...
            if len(ngh_idx) > 0:
                if self.sampling == 0:
                    ...
                    handel_time = (delta_t)*2
                    weights = np.exp(handel_time) + 1e-9                    
...
def load_checkpoint(checkpoint_dir, device='cpu', args=None):
    if os.path.isfile(checkpoint_dir):
        ...
        model = xERTE(nf, contents.num_entities, contents.num_relations, contents.timestamps, contents.ent_time_set, use_time_embedding=not args.no_time_embedding, **kwargs)
...
def new_checkpoint(save_dir, args, struct_time):
    checkpoint_dir = 'checkpoints_{}_{}_{}_{}_{}_{}'.format(
        ...
        struct_time.tm_min,
        struct_time.tm_sec)
    CHECKPOINT_PATH = os.path.join(save_dir, 'Checkpoints', args.dataset, checkpoint_dir)
