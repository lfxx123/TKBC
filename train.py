'''
Descripttion: Train and eval model
version: 1.0
Author: Assassin 567
Date: 2021-11-03 10:40:02
LastEditors: Hello KG
LastEditTime: 2021-11-06 11:15:29
'''
...
parser.add_argument('--DP_num_edges', type=int, default=[200,15,15], nargs='+', help='number of edges at each sampling') # nargs='+' 
...
if __name__ == "__main__":
    ...
    best_epoch = 0
    best_val = 0
    with open(os.path.join(CHECKPOINT_PATH, 'train.log'), 'w') as log_file:
        ...
        for epoch in range(start_epoch, args.epoch):
            ...
            for batch_ndx, sample in tqdm(enumerate(train_data_loader)):
                ...
                entity_att_score, entities, visited_nodes_reg, loss_error = model(sample,train_flag=True) #, p2o)
                ...
                writer.add_scalar('loss/train_'+str(epoch),loss.item(), batch_ndx)
                writer.add_scalar('loss/reg_'+str(epoch),visited_nodes_reg.item(), batch_ndx)
                writer.add_scalar('loss/loss-error_'+str(epoch),loss_error.item(), batch_ndx)

                loss = loss + visited_nodes_reg*args.reg_weight + args.loss_error_weight*loss_error#
            ...
            if epoch % 1 == 0:
                ...
                val_data_loader = DataLoader(val_inputs, batch_size=args.batch_size//4, collate_fn=collate_wrapper,
                                            pin_memory=False, shuffle=True)
                ...
                for batch_ndx, sample in enumerate(val_data_loader):
                    ...
                    entity_att_score, entities,_ = model(sample) #,p2o)

                    loss = model.loss(entity_att_score, entities, target_idx_l, args.batch_size//4,
                ...
                print(
                    "Filtered performance (time dependent): Hits@1: {:.4f}, Hits@3: {:.4f}, Hits@10: {:.4f}, MRR: {:.4f}".format(
                        hit_1_fil_t / num_query,
                        hit_3_fil_t / num_query,
                        hit_10_fil_t / num_query,
                        MRR_total_fil_t / num_query))
                print(
                    "Filtered performance (time independent): Hits@1: {:.4f}, Hits@3: {:.4f}, Hits@10: {:.4f}, MRR: {:.4f}".format(
                        hit_1_fil / num_query,
                        hit_3_fil / num_query,
                        hit_10_fil / num_query,
                        MRR_total_fil / num_query))
                print(
                    "Raw performance: Hits@1: {}, Hits@3: {:.4f}, Hits@10: {:.4f}, Hits@Inf: {:.4f}, MR: {:.4f}, MRR: {:.4f}, degree: {:.4f}".format(
                        hit_1 / num_query,
                        hit_3 / num_query,
                        hit_10 / num_query,
                        found_cnt / num_query,
                ...
                log_file.write(
                    "Filtered performance (time dependent): Hits@1: {}, Hits@3: {}, Hits@10: {}, MRR: {} \n".format(
                        hit_1_fil_t / num_query,
                        hit_3_fil_t / num_query,
                        hit_10_fil_t / num_query,
                        MRR_total_fil_t / num_query))
                ...
                log_file.write(
                    "Filtered performance (time independent): Hits@1: {}, Hits@3: {}, Hits@10: {}, MRR: {} \n".format(
                        hit_1_fil / num_query,
                        hit_3_fil / num_query,
                        hit_10_fil / num_query,
                        MRR_total_fil / num_query))
                ...
                log_file.write(
                    "Raw performance: Hits@1: {}, Hits@3: {}, Hits@10: {}, Hits@Inf: {}, MR: {}, MRR: {}, degree: {} \n".format(
                        hit_1 / num_query,
                        hit_3 / num_query,
                        hit_10 / num_query,
                        found_cnt / num_query,
                ...
                if found_cnt:
                    print("Among Hits@Inf: Hits@1: {:.4f}, Hits@3: {:.4f}, Hits@10: {:.4f}, MR: {:.4f}, MRR: {:.4f}, degree: {:.4f}".format(
                        hit_1 / found_cnt,
                        hit_3 / found_cnt,
                        hit_10 / found_cnt,
                        MR_found / found_cnt,
                    ...
                    log_file.write("Among Hits@Inf: Hits@1: {}, Hits@3: {}, Hits@10: {}, MR: {}, MRR: {}, degree: {} \n".format(
                            hit_1 / found_cnt,
                            hit_3 / found_cnt,
                            hit_10 / found_cnt,
                            MR_found / found_cnt,
    ...
    os.system("python ./eval.py --load_checkpoint {}/{}/checkpoint_{}.pt --whole_or_seen {} --device {} --mongo".format(args.dataset,
                                                    checkpoint_dir, best_epoch, args.whole_or_seen, args.device))
